...
const resolveStrictCalendars = (account, location, typeInfo) => {
  ...
  if (typeCalendarIds.length) {
    ...
    finalIds = configuredIds.filter((id) => allowed.has(id));
    disallowed = configuredIds.filter((id) => !allowed.has(id));
    if (!finalIds.length) {
      // If configured IDs are not part of the allow list, fall back to the allow list
      // so operators see which IDs Acuity expects.
      finalIds = [];
    }
  }
