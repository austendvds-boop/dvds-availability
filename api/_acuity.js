...
const resolveStrictCalendars = (account, location, typeInfo) => {
  ...
  const uniqueConfigured = configuredIds.slice();
  const disallowed = [];
  let finalIds = [];
  let source = "config";

  if (uniqueConfigured.length) {
    if (typeCalendarIds.length) {
      const allowed = new Set(typeCalendarIds);
      const intersection = uniqueConfigured.filter((id) => allowed.has(id));
      if (intersection.length) {
        finalIds = intersection;
        source = "config";
        disallowed.push(...uniqueConfigured.filter((id) => !allowed.has(id)));
      } else {
        finalIds = typeCalendarIds.slice();
        source = "type";
        disallowed.push(...uniqueConfigured);
      }
    } else {
      finalIds = uniqueConfigured.slice();
      source = "config";
    }
  } else if (typeCalendarIds.length) {
    finalIds = typeCalendarIds.slice();
    source = "type";
  }

  return {
    configuredIds: uniqueConfigured,
    unresolved,
    typeCalendarIds,
    disallowed,
    finalIds,
