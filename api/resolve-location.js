...
module.exports = async (req, res) => {
  try {
    ...
    const configuredIds = (cfg[account] && cfg[account][location]) ? cfg[account][location].map(Number).filter(Boolean) : [];
    if (!configuredIds.length) {
      return res.status(404).json({ ok:false, account, location, appointmentTypeId, error:"No calendars configured for this location (strict)" });
    }
    ...
    if (!intersection.length) {
      return res.status(400).json({ ok:false, account, location, appointmentTypeId, configuredIds, typeCalendarIds, error:"Configured calendars aren’t enabled for this type in Acuity" });
    }

    res.status(200).json({ ok:true, account, location, appointmentTypeId, configuredIds, typeCalendarIds, intersection });
  } catch (e) {
